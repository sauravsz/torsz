    ...
    <div className="bg-surface-dark border border-surface-dark-elevated rounded-2xl p-5 shadow-md space-y-4 animate-keyframe-fade-up">
      <div className="flex items-center justify-between border-b border-surface-dark-elevated pb-3">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 bg-surface-dark-soft p-1 rounded-lg border border-surface-dark-elevated">
            {tableaus.map((t, idx) => (
              <button
                key={idx}
        ...
        {current.enteringVar && current.leavingVar && (
          <div className="text-xs font-mono flex items-center gap-2 text-on-dark-soft">
            <span>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left font-mono text-xs border-collapse">
          <thead>
            <tr className="border-b border-surface-dark-elevated bg-surface-dark-soft text-on-dark-soft">
              <th className="p-2.5 border-r border-surface-dark-elevated text-center">Basic</th>
              {current.headers.map((h, colIdx) => (
                <th
                  key={h}
                  className={`p-2.5 border-r border-surface-dark-elevated text-center ${
                    current.pivotColIdx === colIdx ? "bg-primary/20 text-primary font-bold" : "text-on-dark"
                  }`}
                >
                  {h}
                </th>
              ))}
              <th className="p-2.5 text-center text-muted-soft">Ratio</th>
            </tr>
          </thead>
          <tbody>
            {current.rows.map((row, rowIdx) => {
              ...
              return (
                <tr
                  key={rowIdx}
                  className={`border-b border-surface-dark-elevated/50 ${
                    isPivotRow ? "bg-primary/10" : ""
                  }`}
                >
                  <td className="p-2.5 font-bold text-center border-r border-surface-dark-elevated text-accent-teal">
                  ...
                  {row.map((val, colIdx) => {
                    ...
                    return (
                      <td
                        key={colIdx}
                        className={`p-2.5 text-center border-r border-surface-dark-elevated ${
                          isPivotCell
                  ...
                  <td className="p-2.5 text-center text-muted-soft font-mono">
            ...
            {/* Z-Row */}
            <tr className="bg-surface-dark-elevated/70 font-bold border-t-2 border-surface-dark-elevated">
              <td className="p-2.5 text-center border-r border-surface-dark-elevated text-primary">
                Z (z_j - c_j)
              </td>
              {current.zRow.map((val, colIdx) => (
                <td
                  key={colIdx}
                  className={`p-2.5 text-center border-r border-surface-dark-elevated ${
                    current.pivotColIdx === colIdx ? "bg-primary/30 text-primary" : "text-on-dark"
                  }`}
                >
                  {Math.round(val * 1000) / 1000}
                </td>
              ))}
              <td className="p-2.5 text-center text-muted-soft">-</td>
            </tr>
          </tbody>
        </table>
      </div>
