...
import { RefreshCw, FileSpreadsheet, History, Download, Search, Menu } from "lucide-react";
  ...
  return (
    <header className="h-14 bg-canvas border-b border-hairline px-4 flex items-center justify-between select-none z-20">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        {onToggleSidebar && (
          <button
            ...
            className="md:hidden p-1.5 text-muted hover:text-ink hover:bg-surface-soft rounded-lg border border-hairline transition-colors"
            title="Toggle Menu"
          >
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 cursor-pointer">
          ...
          <span className="font-editorial-serif text-2xl sm:text-[28px] font-bold tracking-tight text-ink">
            torsz
          </span>
        </div>
      </div>
      ...
      <button
        type="button"
        onClick={onOpenCommandPalette}
        className="sm:hidden p-2 text-muted hover:text-ink bg-surface-soft rounded-lg border border-hairline transition-colors"
        title="Search (⌘K)"
      >
